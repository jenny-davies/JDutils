...
`correct_filepath_format`: convert a filepath with folders separated by backslashes into a filepath with forward slashes which can be read into R.

`set_debugging_options`: set options so any error messages are automatically used as prompts for ChatGPT via the `searcher` package, with the "debugging" prompt used.

`stop_quietly`: stop a process without throwing an error message, when the desired behaviour of the programme is to stop.
